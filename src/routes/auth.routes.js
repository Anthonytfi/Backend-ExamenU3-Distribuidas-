...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!full_name) return res.status(400).json({ message: "full_name es requerido" });
    ...
    // opcional: devolver token y user para flujo más cómodo en RN
...
router.post("/auth0", async (req, res) => {
  try {
    ...
    const name = decoded.name || decoded.nickname || "Usuario";
